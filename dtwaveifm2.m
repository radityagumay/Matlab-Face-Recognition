function Z = dtwaveifm2(Yl,Yh,biort,qshift);
...
%
% 
% For example:  Z = dtwaveifm2(Yl,Yh,'antonini','qshift_c');
...
while current_level >= 2; ;  %this ensures that for level -1 we never do the following
   lh = c2q(Yh{current_level}(:,:,[1 6]));
   hl = c2q(Yh{current_level}(:,:,[3 4]));
   hh = c2q(Yh{current_level}(:,:,[2 5]));
