function [Yl,Yh,Yscale] = dtwavexfm2(X,level,biort,qshift);

% Function to perform a n-level DTCWT-2D decompostion on a 2D matrix X
%
% [Yl,Yh,Yscale] = dtwavexfm2(X,no_levels,'Names of Biort','Name of q_shift');
%
...
%     Yscale -> This is an OPTIONAL output argument, that contains a cell array for each 
%               real lowpass coefficients at every scale.
%
% 
% Example: [Yl,Yh] = dtwavexfm2(X,3,'antonini','qshift_c');
...
Yh=cell(level,1);
if nargout == 3
   Yscale=cell(level,1);   %this is only required if the user specifies a third output component.
end
...
if level >= 2;
   for count = 2:level;
      ...
      Yh{count} = zeros([size(LoLo)/2  6]);
      Yh{count}(:,:,[1 6]) = q2c(coldfilt(Hi,h0b,h0a).');	% Horizontal
      Yh{count}(:,:,[3 4]) = q2c(coldfilt(Lo,h1b,h1a).');	% Vertical
      Yh{count}(:,:,[2 5]) = q2c(coldfilt(Hi,h1b,h1a).');	% Diagonal   
      S = [ size(LoLo) ;S];
      if nargout == 3
         Yscale{count} = LoLo;
      end
